...
def re_rank(data):
    data['weighted_score'] = (data['checkworthy']*data['checkworthy_numeric']*checkworthy_weight_slider)
    if st.session_state['user_defined_facet']:
        for item in st.session_state['user_defined_facet']:
            new_facet = item['facet_name']
...
with st.sidebar:
    ...
    weight_slider_list = [checkworthy_weight_slider]
    criteria_list = ['checkworthy']
...
if checkworthy:
    if checkworthy_select:
        df_filter_data = filter_data(df_filter_data, 'checkworthy')
        df_filter_data = df_filter_data[df_filter_data['checkworthy_numeric'].between(st.session_state['checkworthy_slider'][0], st.session_state['checkworthy_slider'][1])]
